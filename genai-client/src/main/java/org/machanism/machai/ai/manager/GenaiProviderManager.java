 ...
 *
 * <p>
 * The {@code GenaiProviderManager} offers static methods to instantiate {@link Genai} and {@link EmbeddingProvider}
 * implementations based on a provider/model string, using Java reflection. Providers are expected to follow a
 * conventional package and class naming pattern, or may be specified by fully qualified class name.
 * </p>
 *
 * <h2>Provider Naming Convention</h2>
 * <ul>
 *   <li>Provider and model are specified as {@code Provider:Model} (e.g., {@code OpenAI:gpt-4}).</li>
 *   <li>If the provider name contains a dot ({@code .}), it is treated as a fully qualified class name.</li>
 *   <li>Otherwise, the provider is resolved using the pattern {@code org.machanism.machai.ai.provider.{provider}.{Provider}Provider}.</li>
 * </ul>
 *
 * <h2>Usage Example</h2>
 * <pre>{@code
 ...
 *
 * <p>
 * If the provider cannot be found or instantiated, an {@link IllegalArgumentException} is thrown.
 * </p>
 *
...
public class GenaiProviderManager {
    ...
    /**
     * Dynamically loads and initializes a {@link Genai} provider based on the specified provider/model string.
     *
     * <p>
     * The provider name and model are parsed from the input string (format: {@code Provider:Model}).
     * The provider class is resolved using a conventional naming pattern or as a fully qualified class name.
     * The provider is instantiated and initialized with the specified model and configuration.
     * </p>
     *
     ...
     * @return the initialized {@link Genai} provider instance, or {@code null} if the provider name is blank
     * @throws IllegalArgumentException if the provider name is invalid, or if the provider cannot be found or instantiated
     */
    ...
    /**
     * Dynamically loads and initializes an {@link EmbeddingProvider} based on the specified provider/model string.
     *
     * <p>
     * The provider name and model are parsed from the input string (format: {@code Provider:Model}).
     * The provider class is resolved using a conventional naming pattern or as a fully qualified class name.
     ...
     * </p>
     *
     * @param embeddingModel the provider/model string (e.g., {@code OpenAI:embedding-model})
     * @param conf           the configuration object for provider initialization
     * @return the initialized {@link EmbeddingProvider} instance, or {@code null} if the provider name is blank
     * @throws IllegalArgumentException if the provider cannot be found, does not implement {@link EmbeddingProvider}, or cannot be instantiated
     */
     ...
     *
     * <p>
     * If the provider name contains a dot ({@code .}), it is treated as a fully qualified class name.
     * Otherwise, the provider is resolved using the specified pattern. If the class is not loadable,
     * a fallback naming convention is used.
     * </p>
     *
     * @param providerName        the provider name (e.g., {@code OpenAI})
     * @param conventionalPattern the naming pattern (e.g., {@code org.machanism.machai.ai.provider.%s.%sProvider})
