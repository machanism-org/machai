...
class NoneProviderTest {
    ...
    /**
     * Tests that addTool is a no-op and does not throw.
     */
    @Test
    void testAddToolDoesNotFail() {
        provider.addTool("test", "desc", mock(Function.class));
    }

    /**
     * Tests that addFile methods are no-ops and do not throw.
     * @throws Exception if the file/URL cannot be handled (should not happen)
     */
    @Test
    void testAddFileMethodsAreNoOps() throws Exception {
        provider.addFile(new File("file.txt"));
        provider.addFile(new URL("http://test.com/file.txt"));
    }
