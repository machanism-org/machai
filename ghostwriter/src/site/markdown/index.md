...
| `-t` | `--threads` | Optional (`true`/`false`) | Enable multi-threaded processing. If present without a value, the CLI defaults the value to `true`. | `true` |
...
| `-e` | `--excludes` | Yes (dir[,dir...]) | Directories to exclude from processing. Comma-separated list. (The help text mentions repeating the option, but the current implementation uses only the last occurrence.) | From `gw.properties` key `excludes` (comma-separated); otherwise none |
