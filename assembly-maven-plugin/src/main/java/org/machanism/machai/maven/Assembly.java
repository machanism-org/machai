...
/**
 * Maven plugin {@link org.apache.maven.plugin.Mojo} that implements the
 * {@code assembly} goal.
 *
 * <p>
 ...
 * </p>
 * <ol>
 * <li>Read an assembly prompt from {@link #assemblyPromptFile} (when present)
 * or ask for it interactively.</li>
 * <li>Use {@link #pickChatModel} to recommend candidate libraries
 ...
 * <ul>
 * <li>{@code -Dassembly.genai} (default {@code OpenAI:gpt-5}) &ndash; Provider
 * id for the assembly phase.</li>
 * <li>{@code -Dpick.genai} (default {@code OpenAI:gpt-5-mini}) &ndash; Provider
 * id for the library recommendation (picker) phase.</li>
 * <li>{@code -Dassembly.prompt.file} (default {@code project.txt}) &ndash; File
 * containing the prompt; if absent, the prompt is requested interactively.</li>
 * <li>{@code -Dassembly.score} (default {@code 0.9}) &ndash; Minimum score
 * required for a recommended library to be listed/used.</li>
 * <li>{@code -Dbindex.register.url} (optional) &ndash; Registration/lookup
...
public class Assembly extends AbstractMojo {
	 ...
	 * </p>
	 * <ol>
	 * <li>Read the prompt from {@link #assemblyPromptFile} when it exists;
	 * otherwise prompt the user.</li>
	 * <li>Initialize the picker GenAI provider and apply standard system function
	 * tools.</li>
	 * <li>Use {@link Picker} to recommend libraries and log recommendations to the
	 * build output.</li>
	 * <li>Run the {@link ApplicationAssembly} workflow in {@link #basedir} using
	 * the assembly GenAI provider.</li>
	 * <li>Optionally enter an interactive prompt loop (skipped for
	 * {@link NoneProvider}).</li>
	 * </ol>
	 *
