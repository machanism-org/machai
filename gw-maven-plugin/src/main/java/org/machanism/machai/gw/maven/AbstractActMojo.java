...
public abstract class AbstractActMojo extends AbstractGWMojo {
	 ...
	 * act prompt is resolved. It is shared by all mojo instances in the current
	 * class loader.
	 */
	...
	public void performAct(String actPrompt) throws MojoExecutionException {
		...
		ActProcessor actProcessor = new ActProcessor(basedir, model, configuration) {
			 ...
			 * @throws FileNotFoundException if a required project descriptor cannot be
			 *                               found while creating the layout
			 */
			@Override
			...
			/**
			 * Collects user input when the selected act requires an interactive value.
			 *
			 * @return the text entered by the user, including any continued lines
			 * @throws IllegalArgumentException if the interactive prompt cannot be read
			 */
			@Override
	 ...
	 * document scanning. Usage statistics are initialized before configuration and
	 * logged even when scanning fails.
	 *
	 ...
	 * @throws MojoExecutionException if scanning fails because of I/O errors;
	 *                                prompt failures are propagated by invoked
	...
	/**
	 * Ensures an act prompt is stored in Maven user properties. A configured value
	 * takes precedence over interactive input; entering the exit command leaves no
	 * new act property in the session.
	 *
