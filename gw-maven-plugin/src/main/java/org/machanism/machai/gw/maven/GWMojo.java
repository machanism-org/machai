 ...
 * {@code <basedir>${project.basedir}</basedir>}.</dd>
 * <dt>{@code path}</dt>
 ...
 * configuration:
 * {@code <excludes><exclude>target/**</exclude></excludes>}.</dd>
 * <dt>{@code project}</dt>
 * <dd>The current Maven project, injected from {@code ${project}} when a
 * project is present. Example plugin configuration:
 * {@code <project>${project}</project>}.</dd>
 * <dt>{@code session}</dt>
 ...
 * resolution. Example plugin configuration:
 * {@code <settings>${settings}</settings>}.</dd>
 * <dt>{@code serverId}</dt>
 ...
 * including: - A clear description of all Maven plugin parameters. - Examples
 * of usage for each parameter. - Information inherited from any superclass or
 * parent class. - Escape the closing javadoc tag in javadoc content, as it was
 * breaking javadoc compilation. - Note: When using Maven parallel execution,
 * the modules will be processed by `gw` instead of the Maven reactor. -
 * **Processing Order for the `gw:act` Goal:** - The `gw:act` goal processes
 * files in reverse order, similar to the Ghostwriter CLI. - Sub-modules are
 * processed first, followed by parent modules. - This goal can be executed
 * without a `pom.xml` file. - **Tip:** For improved performance, use Maven
 * parallel execution: `mvn -T 4 gw:gw`.
 * 
 * @since 1.1.2
 */
...
public class GWMojo extends AbstractGWMojo {
	...
	/**
	 * Resolves the Maven project from the current session that matches the artifact
	 * id of the supplied effective model.
	 *
