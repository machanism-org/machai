...
| [GenAI Client](genai-client/) | GenAI Client is a Java library designed for seamless integration with Generative AI providers. It offers foundational prompt management, optional tool/function calling, file context, and (provider-dependent) embeddings through a small, stable API (`GenAIProvider`). The main benefit is provider portability: you can swap or combine backends by changing configuration rather than application code. |
| [Bindex Core](bindex-core/) | Bindex Core (`bindex-core`) is the foundational library for bindex metadata: generating it for artifacts you produce, reading and validating it from dependencies, and combining it across modules to drive higher-level tooling. It provides the canonical model and core APIs for generating, validating, merging, and consuming bindex descriptors used for library discovery, selection, and metadata-driven assembly workflows. |
